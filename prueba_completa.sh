...
echo 'CPU.config'
sed -i "s/RETARDO=\([0-9\.]*\)/RETARDO=5/" $DIR_CPU

echo 'DIEGO.config'
sed -i "s/TRANSFER_SIZE=\([0-9\.]*\)/TRANSFER_SIZE=32/" $DIR_DIEGO

echo 'FM9.config'
sed -i "s/MODO=\([A-Z]*\)/MODO=SEG/" $DIR_FM9
sed -i "s/TAMANIO=\([0-9]*\)/TAMANIO=4096/" $DIR_FM9
...
echo 'SAFA.config'
sed -i "s/ALGORITMO=\([A-Z]*\)/ALGORITMO=IOBF/" $DIR_SAFA
sed -i "s/QUANTUM=\([0-9]*\)/QUANTUM=2/" $DIR_SAFA
sed -i "s/MULTIPROGRAMACION=\([0-9]*\)/MULTIPROGRAMACION=5/" $DIR_SAFA
sed -i "s/RETARDO_PLANIF=\([0-9]*\)/RETARDO_PLANIF=10/" $DIR_SAFA
