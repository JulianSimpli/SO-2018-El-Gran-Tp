...
echo 'CPU.config'
sed -i "s/RETARDO=\([0-9\.]*\)/RETARDO=100/" $DIR_CPU
...
echo 'MDJ.config'
sed -i "s/RETARDO=\([0-9\.]*\)/RETARDO=1000/" $DIR_MDJ

echo 'SAFA.config'
sed -i "s/ALGORITMO=\([A-Z]*\)/ALGORITMO=RR/" $DIR_SAFA
sed -i "s/QUANTUM=\([0-9]*\)/QUANTUM=2/" $DIR_SAFA
sed -i "s/MULTIPROGRAMACION=\([0-9]*\)/MULTIPROGRAMACION=3/" $DIR_SAFA
