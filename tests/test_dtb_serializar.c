...
context(test_dtb) {
	...
	describe("serializa dtb") {
		...
		before {
			...
			otro_archivo->segmento = 4;
			otro_archivo->pagina = 1;
		...
		it("Puede serializar y deserializar archivo") {
			...
			should_int(tamanio_serializado) be equal to(27);
			...
			should_int(deserializado->segmento) be equal to(2);
			should_int(deserializado->pagina) be equal to(5);
			should_string(deserializado->path) be equal to("ejemplo.txt");

			int tamanio_2 = 0;
			void *serializado_2 = DTB_serializar_archivo(otro_archivo, &tamanio_2);
			should_int(tamanio_2) be equal to(32);
			...
			should_int(deserializado_2->segmento) be equal to (4);
			should_int(deserializado_2->pagina) be equal to (1);
		...
		it("Puede serializar y deserializar lista") {
			...
			should_int(tamanio_lista_serializada) be equal to(59);
		...
		it("Puede serializar y deserializar dtb entero") {
			...
			should_int(tamanio_dtb) be equal to(79);
