...
void *DTB_serializar_archivo(ArchivoAbierto *archivo, int *desplazamiento)
{
    ...
    void *serializado = malloc(tamanio * 4 + tamanio_direccion);

    memcpy(serializado + *desplazamiento, &archivo->cantLineas, tamanio);
    *desplazamiento += tamanio;

    memcpy(serializado + *desplazamiento, &archivo->segmento, tamanio);
    *desplazamiento += tamanio;

    memcpy(serializado + *desplazamiento, &archivo->pagina, tamanio);
    *desplazamiento += tamanio;
...
ArchivoAbierto *DTB_leer_struct_archivo(void *data, int *desplazamiento)
{
    ...
    *desplazamiento += tamanio;
    ...
    memcpy(&archivo->segmento, data + *desplazamiento, tamanio);
    *desplazamiento += tamanio;

    memcpy(&archivo->pagina, data + *desplazamiento, tamanio);
    *desplazamiento += tamanio;
...
ArchivoAbierto *_DTB_crear_archivo(u_int32_t cant_lineas, u_int32_t segmento, u_int32_t pagina, char *path)
{
    ArchivoAbierto *archivo = malloc(sizeof(ArchivoAbierto));
    archivo->cantLineas = cant_lineas;
    archivo->segmento = segmento;
    archivo->pagina = pagina;
...
void DTB_agregar_archivo(DTB *dtb, u_int32_t cant_lineas, u_int32_t segmento, u_int32_t pagina, char *path)
{
    ArchivoAbierto *archivo = _DTB_crear_archivo(cant_lineas, segmento, pagina, path);
